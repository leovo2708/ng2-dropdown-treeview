...
import { DropdownTreeviewModule, TreeviewConfig, TreeviewComponent } from '../index';
...
describe('TreeviewComponent', () => {
    ...
    it('should display "No items found" if no items provided', () => {
        const fixture = createTestComponent('<leo-treeview></leo-treeview>');
        expect(fixture.nativeElement.textContent.trim()).toBe('No items found');
    });

    it('should handle correct configuration', () => {
        const fixture = createTestComponent('<leo-treeview [items]="items" [config]="config"></leo-treeview>');
    ...
    it('search "internet" -> uncheck "Internet" -> clear search -> Expect: item "Networking" must be unchecked', async(() => {
        const fixture = createTestComponent('<leo-treeview [items]="items" [config]="config"></leo-treeview>');
        fixture.whenStable().then(() => {
            const textboxFilter = queryTextboxFilter(fixture.debugElement);
            ...
            fixture.whenStable().then(() => {
                ...
                fixture.whenStable().then(() => {
                    ...
                    fixture.whenStable().then(() => {
                        checkboxItems = queryCheckboxItems(fixture.debugElement);
                        let networkingCheckBox = checkboxItems.filter(item =>
